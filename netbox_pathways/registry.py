...
class MapLayerRegistry:
    ...
    def register(self, layer: MapLayerRegistration) -> None:
        ...
        if layer.geometry_type not in _VALID_GEOMETRY_TYPES:
            raise ValueError(f"Invalid geometry_type '{layer.geometry_type}'. Must be one of {_VALID_GEOMETRY_TYPES}.")
        if layer.source not in _VALID_SOURCES:
            raise ValueError(f"Invalid source '{layer.source}'. Must be one of {_VALID_SOURCES}.")
        ...
        if (
            ...
            raise ValueError(f"color_field '{layer.style.color_field}' must be included in feature_fields.")
