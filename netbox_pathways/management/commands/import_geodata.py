        ...
        try:
            ...
            if children_specs:
                for child_spec in children_specs:
                    ...
                    for field in ('tenant', 'start_structure', 'end_structure',
                    ...
                    if child_name_tmpl:
                        ...
                        child_kwargs[name_field] = child_name_tmpl.replace(
                            '{_parent_name}', kwargs.get(parent_name_field, ''),
                        )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for src, spec in fk_specs.items():
            ...
            if related_model is None:
                raise CommandError(
                    f"Field '{target}' on {model_class.__name__} is not a ForeignKey"
                )
        ...
        if options['update']:
            ...
            for j, kwargs in enumerate(all_kwargs):
                try:
                    ...
                    _, was_created = model_class.objects.update_or_create(
                        **{name_field: lookup_val}, defaults=update_kwargs,
                    )
                    if was_created:
                        created_count += 1
                    else:
            ...
            with ProcessPoolExecutor(
                ...
                futures = {
                    executor.submit(
                        _save_batch, model_name, chunk, geom_field,
                ...
                for future in as_completed(futures):
                    ...
                    rate = created_count / elapsed if elapsed > 0 else 0
                    self._log(
                        f'  {created_count:,}/{total:,} '
                        f'({rate:,.0f}/s, {len(save_errors)} errors)...'
                    )
    ...
    def _report_misses(self, fk_misses):
        ...
        self._log(
            f'  FK lookup misses: {total_misses} features skipped '
            f'({len(fk_misses)} distinct names)',
