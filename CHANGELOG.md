...
- The `feat/aerial-span-per-side-attachment-height` branch is also in-flight and has claimed migration `0017_aerialspan_attachment_height_per_side` against the same predecessor `0016_cablesegment_lashed_with`. Whichever branch is merged first will keep its `0017` filename; the second must be renumbered to `0018` (and its `dependencies` updated to point at the first branch's `0017`) before it can be applied cleanly.
